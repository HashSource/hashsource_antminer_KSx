int __fastcall main(int a1, const char **a2, char **a3)
{
  ...
  if ( strstr(*a2, "daemonc") )
    return sub_10EC4(a2[1]);
  if ( strstr(v4, "daemons") )
  {
    v5 = socket(2, 1, 0);
    if ( v5 == -1 )
      _assert_fail("sockfd_server != -1", "update-daemon.c", 0xA6u, "ProcessServer");
    addr.sa_family = 2;
    ...
    *(_WORD *)addr.sa_data = v7;
    ...
    while ( pthread_create(&newthread, 0, (void *(*)(void *))sub_10C94, &arg) != -1 );
    fwrite("pthread_create error!\n", 1u, 0x16u, (FILE *)stderr);
    if ( shutdown(v5, 1) == -1 )
      _assert_fail("ret != -1", "update-daemon.c", 0xD0u, "ProcessServer");
